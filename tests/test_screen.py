...
def test_grid_layout_cells_and_flags():
    ...
    i = r.name
    assert g.attrs["best"].loc[i, "pinnacle"] and not g.attrs["best"].loc[i, "draftkings"]
    assert g.attrs["off_line"].loc[i, "fanduel"] and not g.attrs["off_line"].loc[i, "draftkings"]
    ...
    assert not g.attrs["off_line"].loc[m.name].any()                                # no numbers on a moneyline
    assert list(g.attrs["best"].columns) == ["draftkings", "fanduel", "pinnacle"] and len(g.attrs["best"]) == len(g)
...
def test_grid_exclude_empty_and_props():
    ...
    alt = odds_grid(_board()[lambda d: d.book != "fanduel"])
    r = alt[(alt.market == "spreads") & (alt.pick == "PHI")].iloc[0]
    assert r.draftkings == "-2.5 -110" and r.best == "-105 @ pinnacle" and not alt.attrs["off_line"].loc[r.name].any()
    assert odds_grid(pd.DataFrame()).empty and odds_grid(None).empty
    assert odds_grid(_board().assign(price=np.nan)).empty
    p = odds_grid(load_props_csv(os.path.join(ROOT, "props_template.csv")))
    ...
    assert p.attrs["off_line"].loc[hurts.name].any()                               # the stale book is off the number
